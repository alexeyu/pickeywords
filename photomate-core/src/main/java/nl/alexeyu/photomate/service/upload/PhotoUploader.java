...
public class PhotoUploader {
    ...
    public void uploadPhotos(List<EditablePhoto> photos) {
        ...
        photos.forEach(photo -> photoStocks.forEach(photoStock -> uploadPhoto(photoStock, photo, ATTEMPTS)));
    }
    ...
    private void uploadPhoto(PhotoStock photoStock, EditablePhoto photo, int attemptsLeft) {
        ...
        PhotoToStock photoToStock = new PhotoToStock(photo, photoStock);
        AbstractUploadTask uploadTask = Boolean.valueOf(realUploadProperty)
                ? new FtpUploadTask(photoToStock, attemptsLeft)
                : new FakeUploadTask(photoToStock, attemptsLeft);
